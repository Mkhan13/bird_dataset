'''
Script to web scrape for images of starlings, grackles, blackbirds, and cowbirds from Audubon
'''
import os
import asyncio
            ...
            async with aiohttp.ClientSession() as session:
                for bird, folder in bird_categories.items():
                    ...
                    for img in img_elements:
                        try:
                            #Get the image source (srcset)
                            src = await img.get_attribute('srcset')
                            #Check if the bird name appears in the image source
                            if src and (bird in src.lower()):
